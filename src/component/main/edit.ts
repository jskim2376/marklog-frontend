...
import { Post, PostResponse } from "@/interface/post";
	...
	async connectedCallback() {
		const template = html`
			<div class="d-flex">
				<form class="w-50">
					<header>
						<input id="title" type="text" class="w-100 m fw-bold" placeholder="제목을 입력하세요" style="font-size:2em;" />
						<hr />
						<input name="input" placeholder="태그를 입력후 엔터를 누르세요" id="tagInput" class="w-100" type="hidden" />
					</header>
					<div>
						<textarea id="main" tabindex="0"></textarea>
					</div>
					<footer class="d-flex justify-content-between">
						<button class="btn btn-danger fs-5" type="button" onclick="window.history.back()"><i class=""></i>나가기</button>
				...
				<section class="w-50">
					...
					<br />
					<div id="editor-preview" class="overflow-scroll" style="height:90vh;"></div>
				</section>
			</div>
		`;
		...
		post.addEventListener("click", () => {
			...
			api.putPost(post, postId).then(function (data, textStatus, jqXHR) {
		...
		let api = new Api();
		let postId: number = parseInt(window.location.pathname.split("/")[2]);
		let postResponse: PostResponse = await api.getPost(postId);

		title.value = postResponse.title;
		let preview_title = document.getElementById("preview-title")!;
		preview_title.innerText = title.value;
		let tagList: string[] = [];
		postResponse.tagList.forEach((element) => {
			tagList.push(element.name);
		});
		tagify.addTags(tagList);
		easymde.value(postResponse.content);
	}
}

customElements.define("ml-edit", EditElement);

export function createEdit() {
	return document.createElement("ml-edit");
}
